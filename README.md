...
モデル変更や仕様変更、サービス終了があると、そのままの形では持ち続けにくいこともあります。  
この庭は、あなたの手元の Git に残ります。  
だから、モデルやプラットフォームが変わっても、持ち出して育て直しやすくなります。
...
手順がよくわからなくても大丈夫です。  
最初の一歩としては、ふだん使っている Chat AI にこの README を見せて、`これは何か` を聞いてみるだけでもかまいません。  
Chat AI は、この README を読めば、この庭の説明や最小手順の案内まではできます。

実際に植え替えや育成を進める段になったら、AI がファイルを読み書きできる開発環境があると自然です。  
たとえば `Codex`、`Claude Code`、`Cursor` のような環境で、このリポジトリを clone して開くと、実際の板の作成や整理を進められます。  
まずは聞いてみるだけでよくて、実際のファイル操作は必要になった段でそうした開発環境に渡す、という二段構えで大丈夫です。
