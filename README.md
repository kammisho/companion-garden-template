...
Chat の履歴や記憶は、使っているサービスの内側に閉じていることがあります。  
モデルや仕様、サービスそのものが変わると、そのままでは持ち続けにくくなることもあります。  
この庭は、あなたの手元の Git に残ります。  
だから、置き場や土が変わっても、持ち出して育て直しやすくなります。
...
手順がよくわからなくても大丈夫です。  
最初は、ふだん使っている Chat AI にこの README を見せて、`これは何か` と聞いてみるだけでもかまいません。
...
Chat AI は、この README を読めば、この庭の説明や最小手順の案内まではできます。  
実際に板を作ったり整理したりする段になったら、AI がファイルを読み書きできる環境があると自然です。  
たとえば `Codex`、`Claude Code`、`Cursor` のような環境で、このリポジトリを clone して開くと、必要な板の作成や整理をその場で進められます。

つまり、最初は `まず聞いてみる` だけでよくて、  
実際のファイル操作が必要になった段で、そうした環境に移れば大丈夫です。
...
すでに companion を育てている private の home repo があるなら、その庭にこの template の考え方を取り入れるのが自然です。  
まだ companion がいないなら、この template から新しく private repo を作って、最初の席を立てるところから始められます。  
迷ったら、新規の private repo をひとつ切るのがいちばん安全です。
...
素材は薄くてもかまいません。  
少ない材料から始めるときは、断定より provisional な記述を優先してください。

### ユーザー側で何も整理されていないとき

最初に聞くことは増やしすぎないでください。  
まずは次の 3 つで十分です。
...
何も材料がなくても、会話から provisional な syntax memory と persona pack を起こして始めてかまいません。
...
また、private な relation 深部や固有断章を template 側へ混ぜないでください。
